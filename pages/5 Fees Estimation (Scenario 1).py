...
if "pro_tiers_config_scenario_1" not in st.session_state:
    logic = st.session_state.get("logic_applied_scenario_1", "SUM")
    default_config = get_default_pro_tiers_sum() if logic == "SUM" else get_default_pro_tiers_or()
    st.session_state["pro_tiers_config_scenario_1"] = default_config.copy()
    st.session_state["pro_tiers_applied_version_scenario_1"] = "default"

config = st.session_state["pro_tiers_config_scenario_1"]
logic_label = st.session_state.get("logic_applied_scenario_1", "SUM")
...
with st.expander("1. Calculate Pro Tiers", expanded=False):
    ...
    st.markdown(f"✅ **Applied Config:** `{st.session_state.get('pro_tiers_applied_version_scenario_1','default')}`")
    ...
    if 'pro_tiers_df_vs_scenario_1' not in st.session_state or recalculate:
        ...
        with st.spinner("Calculating Pro Tiers..."):
            ...
            st.session_state['pro_tiers_df_vs_scenario_1'] = pro_tiers_df_vs
    ...
    with st.container():
        ...
        if show_tiers_table and 'pro_tiers_df_vs_scenario_1' in st.session_state:
            ...
            df = st.session_state['pro_tiers_df_vs_scenario_1'].copy()
...
with st.expander("2. Fees Data with Pro Tiers", expanded=False):
    ...
    if 'fees_data_tiers_scenario_1' not in st.session_state or recalculate:
        with st.spinner("Merging Pro Tier data with fees data..."):
            if 'pro_tiers_df_vs_scenario_1' not in st.session_state:
                ...
                pro_tiers_df_vs = st.session_state['pro_tiers_df_vs_scenario_1']
                ...
                # Step 5: Save result
                st.session_state['fees_data_tiers_scenario_1'] = fees_data_tiers
    ...
    with st.container():
        ...
        if show_fees_table and 'fees_data_tiers_scenario_1' in st.session_state:
            st.markdown("### 📋 Sample of Fees Data with Pro Tier Info")

            df = st.session_state['fees_data_tiers_scenario_1'].copy()
...
with st.expander("3. User Distribution Across Categories", expanded=False):
    ...
    if 'tiers_stats_scenario_1' not in st.session_state or recalculate:
        with st.spinner("Computing user category splits by Pro Tier..."):
            ...
            if len(stratified_sample) < 500:
                ...
                df_sample = stratified_sample
            df_sample = df_sample.sort_values(by=['date', 'trader'])
            st.session_state['user_split_sample_scenario_1'] = df_sample
            ...
            st.session_state['tiers_stats_scenario_1'] = tiers_stats
            st.session_state['tiers_stats_snapshot_scenario_1'] = tiers_stats_snapshot

            st.success("✅ Distribution stats calculated.")
    else:
        tiers_stats = st.session_state['tiers_stats_scenario_1']
        tiers_stats_snapshot = st.session_state['tiers_stats_snapshot_scenario_1']
    ...
    with st.container():
        ...
        if show_user_split_sample and 'user_split_sample_scenario_1' in st.session_state:
            st.markdown("### 📋 Sample of User Classification by Volume vs Staking")

            styled_sample = (
                st.session_state['user_split_sample_scenario_1']
                .style.format({
                    'collateral_token_price_avg': '${:,.4f}',
                    'referral_total_rebate_factor': '{:.2%}',
                    'referral_trader_discount_factor': '{:.2%}',
...
with st.expander("4. Calculate New Discounts", expanded=False):
    ...
    if 'fees_data_tiers_scenario_1' not in st.session_state:
        st.error("❌ Please complete Section 2 first (Fees Data with Pro Tiers).")
    else:
        if 'fees_with_discounts_scenario_1' not in st.session_state or recalculate:
            with st.spinner("Calculating combined referral and pro tier rewards..."):
                fees_data_tiers = st.session_state['fees_data_tiers_scenario_1'].copy()
                ...
                st.session_state['fees_with_discounts_scenario_1'] = fees_data_tiers
    ...
    with st.container():
        ...
        if show_discount_sample and 'fees_with_discounts_scenario_1' in st.session_state:
            st.markdown("### 📋 Sample of Combined Discount Calculation")

            df = st.session_state['fees_with_discounts_scenario_1'].copy()
...
with st.expander("5. Recalculate Fees", expanded=False):
    ...
    if 'fees_with_discounts_scenario_1' not in st.session_state:
        st.error("❌ Please complete Section 4 first to calculate discounts.")
    else:
        if 'fees_with_new_fees_scenario_1' not in st.session_state or recalculate:

            with st.spinner("Applying new discount structure to fee values..."):
                fees_data_tiers = st.session_state['fees_with_discounts_scenario_1'].copy()
                ...
                st.session_state['fees_with_new_fees_scenario_1'] = fees_data_tiers
    ...
    with st.container():
        ...
        if show_new_fees_sample and 'fees_with_new_fees_scenario_1' in st.session_state:
            st.markdown("### 📋 Sample of New Fee Recalculation Results")

            df = st.session_state['fees_with_new_fees_scenario_1'].copy()
...
with st.expander("6. Calculate Total Fees and Protocol Fees", expanded=True):
    ...
    if 'fees_with_new_fees_scenario_1' not in st.session_state:
        st.error("❌ Please complete Section 5 first to recalculate fees.")
    else:
        with st.spinner("Calculating monthly and total fee stats..."):
            fees_data_tiers = st.session_state['fees_with_new_fees_scenario_1'].copy()
